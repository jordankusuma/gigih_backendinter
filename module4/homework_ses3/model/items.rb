...
class Item
    ...
    def self.get_item_based_category(categories)
        client = create_db_client
        rawData = client.query("select * from items join item_categories on items.id = item_categories.item_id where item_categories.category_id = #{categories.id} order by items.id")
        items = Array.new
        rawData.each do |data|
